...
class Backend
{
    ...
    */
    ...
    public function addPost($formData)
    {
        ...
        if (!$this->postmanager->exists($formData['title'])) {
            ...
            $this->session->set('show_message', true);
            $this->session->set('message', 'Votre projet est publié !');

            header('location: article-'.$post->idPost());
            return;
        }
        $this->session->set('show_message', true);
        $this->session->set('message', 'Un projet avec le même titre existe déja !');
    }
    ...
    */
    ...
    public function editPost($formData)
    {
        ...
        $post = $this->postmanager->get($post->idPost());
        $this->session->set('show_message', true);
        $this->session->set('message', 'Projet modifié !');
        header('location: article-'.$post->idPost());
    }
    ...
    */
    ...
    public function deletePost($idPost)
    {
        ...
        $this->session->set('show_message', true);
        $this->session->set('message', 'Projet supprimé !');
    ...
    */
    ...
    public function validateUser($idUser)
    {
        ...
        $message = (new \Swift_Message())
            ...
            ->setSubject('Portfolio, votre compte a été validé !')
